...
/*   Updated: 2020/02/23 18:15:14 by llachgar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			fill_matches(char *folder, char *match, t_data **matches)
{
	DIR				*dir;
	...
	{
		if (match[0] != 0 &&
		!ft_memcmp(match, content->d_name, ft_strlen(match)))
		{
			add_new_match(matches, content);
			count++;
		}
		if (match[0] == 0 && ft_memcmp(content->d_name, "..", 2)
		&& ft_memcmp(content->d_name, ".", 1))
		{
			add_new_match(matches, content);
			count++;
		}
...
/*
** complete_or_print: if the number of matches
** is more than one then print it so the user see it
...
void		path_completion(char *patt, t_cmd *l)
{
	...
	count = fill_matches(folder, start_with, &matches);
